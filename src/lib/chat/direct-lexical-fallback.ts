...
import {
  ...
  OPERATIONAL_QUERY_RULES,
  QUERY_HINTS,
  QUESTION_INTENTS,
  RERANK_WEIGHTS,
  RISK_EVIDENCE_IDENTITY_PATTERN,
  RISK_QUERY_HINTS,
  RISK_QUERY_PATTERN,
} from "@/lib/chat/retrieval-tuning";
...
function parseCandidates(rows: unknown, input: DirectLexicalFallbackInput) {
  ...
  rawRows.forEach((value, originalIndex) => {
    ...
    const candidateCodeText = `${docId || ""} ${title} ${content.slice(0, 1200)}`
      .toLowerCase()
      .replace(/-/g, "");
    const visaCodeBoost = questionVisaCodes.some((code) => candidateCodeText.includes(code)) ? RERANK_WEIGHTS.visaCode : 0;
    const rerankScore = round(
      baseScore
        + bodyCoverage * RERANK_WEIGHTS.bodyCoverage
        + headingCoverage * RERANK_WEIGHTS.headingCoverage
        + categoryBoost
        + riskBoost
        + visaCodeBoost
        + operationalBoost
