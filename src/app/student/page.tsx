  ...
  return (
    <main className="min-h-screen bg-muted/30 px-4 py-8">
      <div className="mx-auto max-w-5xl space-y-5">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">{tr("student_my_page", "ko")}</h1>
                ...
                profile.escalationCases.map((caseItem) => (
                  <div key={caseItem.id} className="rounded-md border bg-background px-3 py-2 text-sm">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {caseItem.category} · {formatDate(caseItem.updatedAt)}
                    </div>
                  </div>
                ))
              )}
          ...
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {tr("student_documents", "ko")}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {!profile || profile.documents.length === 0 ? (
                <div className="text-sm text-muted-foreground">
                  {tr("empty_state", "ko")} <Link href="/documents" className="underline">Upload</Link>
                </div>
              ) : (
                profile.documents.map((doc) => (
                  <div key={doc.id} className="rounded-md border bg-background px-3 py-2 text-sm">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">
                      {doc.file?.originalName || "-"} · {formatDate(doc.expiresAt)}
                    </div>
                  </div>
                ))
              )}
