...
type HandoffResponse = {
  tasks: HandoffTask[];
  counts: { total: number; active: number; urgent: number; unassigned: number; contactReady: number };
  error?: string;
};
  ...
  const updateTask = async (action: HandoffAction) => {
    ...
    try {
      const response = await adminFetch("/api/admin/handoffs", {
        method: "PATCH",
        body: JSON.stringify({ id: selected.id, action, assignee, note }),
      });
              ...
              <div className="space-y-5 px-4 pb-6">
                ...
                {canManageOps && (
                  <section className="space-y-3 border-t pt-4">
                    <div><label htmlFor="handoff-assignee" className="mb-1.5 block text-sm font-medium">담당자</label><Input id="handoff-assignee" value={assignee} onChange={(event) => setAssignee(event.target.value)} placeholder="담당자 이메일 또는 이름" /></div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button variant="outline" onClick={() => void updateTask("assign")} disabled={updating || !assignee.trim()}>배정</Button>
