          ...
          {selected && (
            <>
              ...
              <div className="space-y-5 px-4 pb-6">
                ...
                {selected.resolutionCode && <section><h2 className="mb-2 text-sm font-semibold">처리 결과</h2><div className="rounded-md border p-3 text-sm"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{RESOLUTION_LABELS[selected.resolutionCode]}</p>{selected.evaluationActive === false && <Badge variant="outline">평가 검토 대기</Badge>}{selected.evaluationActive === true && <Badge variant="secondary">회귀 평가 활성</Badge>}</div><p className="mt-1 text-xs text-muted-foreground">{selected.resolvedBy || "담당자"} · {formatDate(selected.resolvedAt)}</p>{selected.evaluationCaseId && <p className="mt-2 font-mono text-xs text-primary">{selected.evaluationCaseId}</p>}</div></section>}
