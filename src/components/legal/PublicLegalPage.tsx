    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto w-full max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        <div className="flex flex-wrap gap-4 border-t pt-6 text-sm">
          <Link href={`/${locale}/privacy`} className="inline-flex items-center gap-1 text-primary-strong hover:underline">{copy.privacyLink}<ExternalLink className="h-3.5 w-3.5" /></Link>
          <Link href={`/${locale}/terms`} className="inline-flex items-center gap-1 text-primary-strong hover:underline">{copy.termsLink}<ExternalLink className="h-3.5 w-3.5" /></Link>
        </div>
      </main>
    </div>
  );
