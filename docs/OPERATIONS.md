...
7. Deletion requests are accepted through `POST /api/privacy/delete-request`, which acts only on records the caller **proved** are theirs — an authenticated session, or the signed `kaxi_lead_access` cookie issued when they saved a diagnosis. Nothing in the request body selects records: `leadId` and `contact` are not read as selectors, and an exact `question` is refused with `400`, because many people type the same question and it matched all of their rows. A request carrying no proof is recorded and audited but mutates nothing, pending the contact-verification channel. Every outcome returns the same `202`, so the endpoint cannot be used to ask whether a record exists. Active lead consents are withdrawn for the proven leads.
