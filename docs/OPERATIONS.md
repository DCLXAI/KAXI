...
- `KNOWLEDGE_MONITOR_PERSIST_CANDIDATES`: Candidate-write kill switch. It defaults to `false`, so scheduled and admin monitoring remains audit-only until an operator explicitly sets it to `true`. Keep it `false` during source-parser incidents.
...
- `KAXI_RAG_EMBEDDING_STRATEGY`: A diagnostic signal read only by `rag-system-health.ts`; it no longer drives the chat retrieval path. The chat path is OpenAI-1536d-only and fail-closed: a non-OpenAI query embedding never retrieves — the E5 384d canonical read path and its automatic fallback were removed. `openai-only` is the production value; the historical `openai-primary`/`e5-primary` dual-index fallback no longer exists in code.
...
- `KAXI_QUERY_EMBEDDING_REQUIRED`: Marks system health degraded when the direct OpenAI probe fails. Chat retrieval is fail-closed, so a probe failure surfaces as a runtime failure rather than silently degrading to a lower-quality index.
...
- `high` risk or `urgent` lead-stage handoffs use the `urgent-2h` first-response SLA. Other active handoffs use `standard-24h`. This policy lives in `src/lib/ops/sla-policy.ts` and is shared by all three consult queues (`handoff_tasks`, `PartnerRequest`, `EscalationCase`) so it cannot drift; the risk comparison is case-normalized because `EscalationCase.riskLevel` is an uppercase enum while `handoff_tasks.risk_level` is lowercase.
- `PartnerRequest` and `EscalationCase` carry the SLA on the row (`slaTier`, `slaDueAt`, `slaFirstResponseAt`, `slaBreachAlertedAt`), set when the item is assigned. `PartnerRequest` has no risk column, so it always resolves to `standard-24h`.
...
- Assignment identity, policy version, SLA tier, minutes, start time, due time, and response status are stored in `handoff_tasks.handoff_metadata`. Admin list responses derive `overdue` at read time so an expired pending SLA is visible immediately on load.
- The `/api/ops/sla` watchdog scans all three queues and records an `sla.breached` ops event for each newly breached item (`error` severity when the item is high risk, otherwise `warning`). It also counts items `approaching` their due time (final 25% of the window). It reports only — its sole write is a one-time breach stamp that stops it re-alerting the same item, and each queue is scanned in isolation so one failing queue cannot silence the others. `POST` runs the same scan for an admin on demand, and answers `503` when any queue scan failed so a broken watchdog cannot report "all clear".
- The watchdog runs on two schedules, mirroring the official source monitor: `vercel.json` fires it daily (`45 18 * * *`) and `.github/workflows/sla-watchdog.yml` supplements it hourly. Vercel Hobby only permits daily crons, and an hourly cadence is what makes the `urgent-2h` tier meaningful — a 2-hour SLA is pointless if a breach can sit unreported for a day. The GitHub job posts with `KAXI_ADMIN_API_KEY` (the same secret the source monitor uses) and fails the run on a `503`.
- SLA breaches always land in ops events regardless of channel configuration. Configuring `OPS_ALERT_SLACK_WEBHOOK_URL` or the Resend email variables turns them into real-time delivery; until then the trail is DB/admin-only.
- Assigning a handoff notifies the assignee (`UserNotification`, deduped per task per assignee). Notification failure never fails the assignment.
- Partners see the handoff tasks assigned to them at `/partner` and may record `start` / `contacted` only. The RAG verdict stays admin-only: it feeds `rag_evaluation_cases` and is a retrieval-quality judgment, not a consultation outcome.
...
- Every persisted retrieval with no context, zero results, or a top score within `0.05` of the configured similarity threshold is automatically queued in `handoff_tasks`. Evaluation and health probes are excluded, and one source message can have only one active review task.
- Operators close a review with exactly one structured verdict: `resolved` (the answer and evidence were sufficient), `inaccurate` (the answer needs correction), or `missing_document` (the governed corpus needs a source). The verdict, reviewer, timestamps, linked retrieval, and resulting evaluation case are audited in `rag_review_feedback`.
- A `resolved` review with a masked question and retrieved document IDs becomes an active regression case immediately. `inaccurate` and `missing_document` reviews create inactive cases so an owner can add the corrected expected answer or approved source before activation; the admin queue reports these as evaluation-review pending.
- Product analytics stores only allowlisted event names, a per-tab session UUID, locale, surface, path, and bounded non-sensitive properties. Questions, answers, contacts, filenames, free text, and raw source URLs are rejected. `/admin/analytics` reports diagnosis, chat, citation, handoff, and locale funnels for 7/30/90 days; daily retention deletes events after `PRIVACY_PRODUCT_ANALYTICS_RETENTION_DAYS` (180 by default).
...
7. Deletion requests are accepted through `POST /api/privacy/delete-request`, which acts only on records the caller **proved** are theirs — an authenticated session, or the signed `kaxi_lead_access` cookie issued when they saved a diagnosis. Nothing in the request body selects records: `leadId` and `contact` are not read as selectors, and an exact `question` is refused with `400`, because many people type the same question and it matched all of their rows. A request that names a contact opens a `PrivacyDeletionRequest` and mails a one-time link there; nothing is marked until that link is redeemed, it lasts 24 hours, works once, and a newer request supersedes it. Only `hashPii(contact)` and `sha256(token)` are stored, never the address or the token. This channel needs `SMTP_HOST` and `SMTP_FROM`; without them readiness warns (it does not block deploys) and the other two proofs keep working. A request with no proof and no contact is recorded and audited but mutates nothing. Every outcome returns the same `202`, so the endpoint cannot be used to ask whether a record exists. Active lead consents are withdrawn for the proven leads.
8. Retention selects rows on `retentionProcessedAt IS NULL`, **not** on the `*Redacted` display flags. Those flags are set at write time whenever `DATA_ENCRYPTION_KEY` is configured, so the old selector matched nothing in production and expired ciphertext and lookup hashes survived while the job reported zero rows due. Processed rows are stamped with `retentionProcessedAt` and `retentionVersion`, which makes the sweep idempotent and lets a future policy change find rows handled under a weaker one. Retention is enforced by `POST /api/privacy/retention` for admins and daily Vercel Cron `GET /api/privacy/retention`; lead consent rows are expired when the linked lead reaches deletion or retention expiry. Canonical chat sessions use a rolling 90-day default and privacy deletion removes associated storage objects, messages, retrieval evidence, n8n audit rows, and handoff rows before deleting the session. The same job lists Typebot Results through the dedicated provider token and deletes results older than seven days in bounded batches.
...
14. `/api/knowledge/monitor` checks the official watchlist for recent Immigration Act/Decree/Rule promulgations and effective dates, current Immigration Act/Enforcement Decree/Enforcement Rule text, HiKorea homepage/manual/procedure/form/notice sources, and Ministry of Justice immigration policy news. Vercel Cron calls it daily; GitHub Actions also calls it every 30 minutes in reduced-concurrency source batches to supplement Vercel Hobby cron limits. Candidate writes are fail-closed: both scheduled and admin requests remain audit-only unless `KNOWLEDGE_MONITOR_PERSIST_CANDIDATES=true`. When enabled, the monitor hashes normalized body text rather than volatile response metadata and keeps at most one open `PENDING` candidate per source. If `KNOWLEDGE_MONITOR_ALERT_WEBHOOK_URL` is configured, changed/failed monitor runs send an operations alert with impacted rule/user counts and a link back to `/admin/knowledge`.
...
19. `GET /api/admin/knowledge?page=1&pageSize=25` uses server-side pagination (`pageSize` maximum 100), loads chunk counts without fetching chunk bodies/embeddings, and calculates rule/chat impact for the current page in two shared queries. Paginated admin bulk actions must send `allPendingCandidates: true` to target the complete current `PENDING` set rather than only the visible page.
