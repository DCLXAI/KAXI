-- Restore compatibility rows for databases where the preceding quarantine
-- migration was deployed before the serving projection was ready.
...
SELECT
  ...
  coalesce(original_updated_at, quarantined_at)
FROM public.legacy_rag_chunks_quarantine
ON CONFLICT (id) DO NOTHING;

SELECT setval(
  pg_get_serial_sequence('public.knowledge_chunks', 'id'),
  greatest(coalesce((SELECT max(id) FROM public.knowledge_chunks), 1), 1),
  EXISTS (SELECT 1 FROM public.knowledge_chunks)
);

CREATE OR REPLACE FUNCTION public.kaxi_finalize_legacy_rag_cutover(
  expected_ready_count integer DEFAULT 199
)
...
COMMENT ON FUNCTION public.kaxi_finalize_legacy_rag_cutover(integer) IS
  'Deletes the deprecated knowledge_chunks rows only after governed embeddings and citations reach the expected count.';
