-- Require approved DB RAG documents to be recently checked before production search.

DROP FUNCTION IF EXISTS kaxi_hybrid_knowledge_search(vector, text, text[], integer, integer, integer, text, integer);
...
CREATE OR REPLACE FUNCTION kaxi_hybrid_knowledge_search(
  ...
  review_max_age_days integer DEFAULT 92
)
RETURNS TABLE (
  chunk_id text,
  document_id text,
...
AS $$
  ...
  eligible AS (
    ...
    WHERE d."reviewStatus" = 'APPROVED'
      ...
      AND d."lastCheckedAt" >= now() - make_interval(days => greatest(review_max_age_days, 1))
